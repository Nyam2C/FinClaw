...
> 복잡도: **XL** | 소스 파일: ~14 | 테스트 파일: ~7 | 총 ~21 파일
...
- 7개 핵심 테이블: conversations, messages, memories, memory_chunks, embedding_cache, alerts, market_cache + vec0 가상 테이블 (memory_chunks_vec)
- FTS5 전문 검색: BM25 랭킹 기반 텍스트 쿼리
- sqlite-vec 벡터 검색: 코사인 유사도 기반 시맨틱 검색
- 하이브리드 검색: FTS5 + vector 결과를 Weighted Score Fusion (WSF)으로 병합
- 임베딩 프로바이더: Anthropic (voyage-finance-2), OpenAI (text-embedding-3-small)
- 금융 데이터 TTL 캐싱 (시세 5분, 과거 데이터 1시간)

> Chokidar 파일 감시는 Phase 15로 이관한다.
...
- FinClaw: 동일한 Weighted Score Fusion 구현. 가중치 기본값 vector=0.7, text=0.3
...
| 파일 경로                                      | 역할                                                  | 예상 줄 수 |
| ---------------------------------------------- | ----------------------------------------------------- | ---------- |
| `packages/storage/src/index.ts`                | StorageAdapter 팩토리 + barrel export                 | ~30        |
| `packages/storage/src/database.ts`             | SQLite 데이터베이스 초기화, 스키마 생성, 마이그레이션 | ~200       |
| `packages/storage/src/tables/conversations.ts` | 대화 세션 CRUD (생성, 조회, 메타데이터 갱신)          | ~120       |
| `packages/storage/src/tables/messages.ts`      | 메시지 저장/조회 (대화 이력)                          | ~150       |
| `packages/storage/src/tables/memories.ts`      | 메모리 엔트리 CRUD + 청킹 + 임베딩 연동               | ~200       |
| `packages/storage/src/tables/embeddings.ts`    | 임베딩 캐시 테이블 (프로바이더별 해시 기반 캐시)      | ~80        |
| `packages/storage/src/tables/alerts.ts`        | 가격 알림 테이블 (금융 특화)                          | ~100       |
| `packages/storage/src/tables/market-cache.ts`  | 시장 데이터 TTL 캐시 (금융 특화)                      | ~100       |
| `packages/storage/src/search/fts.ts`           | FTS5 전문 검색 (BM25 랭킹)                            | ~120       |
| `packages/storage/src/search/vector.ts`        | sqlite-vec 벡터 유사도 검색 (코사인)                  | ~130       |
| `packages/storage/src/search/hybrid.ts`        | FTS5 + vector 결합 하이브리드 검색                    | ~100       |
| `packages/storage/src/embeddings/provider.ts`  | EmbeddingProvider 인터페이스 + 팩토리                 | ~80        |
| `packages/storage/src/embeddings/anthropic.ts` | Anthropic voyage-finance-2 임베딩 프로바이더          | ~100       |
| `packages/storage/src/embeddings/openai.ts`    | OpenAI text-embedding-3-small 프로바이더              | ~100       |

### 테스트 파일 (7개)

| 파일 경로                                                  | 테스트 대상                            | 테스트 종류 |
| ---------------------------------------------------------- | -------------------------------------- | ----------- |
| `packages/storage/src/database.test.ts`                    | DB 초기화, 스키마 생성, 마이그레이션   | unit        |
| `packages/storage/src/tables/messages.storage.test.ts`     | 메시지 CRUD, 대화별 이력 조회          | storage     |
| `packages/storage/src/tables/memories.storage.test.ts`     | 메모리 CRUD, 청킹, 임베딩 저장         | storage     |
| `packages/storage/src/search/fts.storage.test.ts`          | FTS5 검색, BM25 랭킹                   | storage     |
| `packages/storage/src/search/vector.storage.test.ts`       | vec0 KNN 검색, 벡터 유사도             | storage     |
| `packages/storage/src/search/hybrid.test.ts`               | 하이브리드 검색 결과 병합, 가중치 조정 | unit        |
| `packages/storage/src/tables/market-cache.storage.test.ts` | 금융 데이터 캐싱, TTL 만료             | storage     |
...
```typescript
// packages/storage/src/database.ts — 데이터베이스 인스턴스
...
// packages/storage/src/tables/conversations.ts
...
// packages/storage/src/tables/memories.ts
// @finclaw/types/storage.ts의 MemoryEntry (id, sessionKey, content, embedding?, type, createdAt, metadata)를
// 공개 인터페이스로 사용한다. 아래는 DB 행 매핑용 내부 타입이다.
interface MemoryRow {
  readonly id: string;
  readonly session_key: string;
  readonly content: string;
  readonly type: 'fact' | 'preference' | 'summary' | 'financial';
...
// packages/storage/src/tables/alerts.ts — 금융 알림
// @finclaw/types/finance.ts의 Alert 타입을 직접 사용한다.
// Alert: id, name?, symbol, condition(AlertCondition), enabled, channelId?,
//        createdAt, lastTriggeredAt?, triggerCount, cooldownMs
// AlertCondition: type(5종), value, field?(price|changePercent|volume|rsi)

// packages/storage/src/tables/market-cache.ts — 시장 데이터 캐시
...
// packages/storage/src/search/hybrid.ts — 하이브리드 검색 내부 결과 타입 (StorageAdapter.searchConversations의 SearchResult와 별개)
...
// packages/storage/src/embeddings/provider.ts — 임베딩 프로바이더
...
// @finclaw/types의 StorageAdapter를 구현하는 팩토리
...
const SCHEMA_DDL = `
  ...
  -- vec0 가상 테이블: 벡터 검색 (임베딩은 여기서 관리)
  CREATE VIRTUAL TABLE IF NOT EXISTS memory_chunks_vec USING vec0(
    chunk_id TEXT PRIMARY KEY,
    embedding float[1024]
  );
  -- 주의: vec0는 CASCADE DELETE 미지원. 삭제 시 수동 처리 필요.

  -- FTS5 가상 테이블: BM25 전문 검색 (trigram 토크나이저로 한국어 부분 매칭 지원)
  ...
  -- 금융 특화: 가격 알림 (@finclaw/types Alert 타입 기반)
  CREATE TABLE IF NOT EXISTS alerts (
    ...
    condition_field TEXT DEFAULT 'price',
...
```typescript
// packages/storage/src/search/fts.ts
import type { DatabaseSync } from 'node:sqlite';
import type { ChunkSearchResult } from './hybrid.js';

/**
 * FTS5 쿼리를 빌드한다.
 * trigram 토크나이저 사용 시 부분 매칭을 지원한다.
...
```typescript
// packages/storage/src/search/vector.ts
import type { DatabaseSync } from 'node:sqlite';
...
export async function searchVector(
  ...
  // 2. sqlite-vec의 KNN 검색 실행 (vec0 가상 테이블 JOIN)
  const stmt = db.prepare(`
    SELECT
      c.id,
      c.memory_id,
...
### 5.4 하이브리드 검색 (Weighted Score Fusion)

```typescript
// packages/storage/src/search/hybrid.ts
import type { ChunkSearchResult, HybridSearchOptions } from './hybrid.js';

/**
 * FTS5와 벡터 검색 결과를 Weighted Score Fusion (WSF)으로 병합한다.
 *
 * 알고리즘:
...
export function mergeHybridResults(
  ...
  const merged = new Map<string, ChunkSearchResult & { combinedScore: number }>();
...
// packages/storage/src/embeddings/anthropic.ts
export class AnthropicEmbeddingProvider implements EmbeddingProvider {
  ...
  async embedBatch(texts: string[]): Promise<number[][]> {
    // Voyage API 배치 크기 (voyage-finance-2 권장)
    const BATCH_SIZE = 50;
...
```typescript
// packages/storage/src/tables/memories.ts (내부 함수)
...
```typescript
// packages/storage/src/tables/market-cache.ts
import type { DatabaseSync } from 'node:sqlite';
...
메모리 저장 흐름:
  사용자 → addMemory(content) → chunkMarkdown(content, 512, 64)
    → embedBatchWithCache(chunks.map(c => c.text))
...
메모리 삭제 흐름:
  deleteMemory(id)
    → SELECT chunk IDs from memory_chunks WHERE memory_id = ?
    → DELETE FROM memory_chunks_vec WHERE chunk_id IN (...)  -- vec0 수동 삭제
    → DELETE FROM memories WHERE id = ?  -- CASCADE로 memory_chunks, FTS 삭제

하이브리드 검색 흐름:
  사용자 쿼리 → search(query, options)
    ├─→ searchFts(db, query, limit) ─────→ FTS5 BM25 결과
    │                                        │
    ├─→ searchVector(db, query, provider) ─→ 벡터 코사인 결과 (memory_chunks_vec JOIN)
    │                                        │
    └─→ mergeHybridResults(vector, fts, weights)
         → WSF (0.7 * vector + 0.3 * fts)
         → minScore 필터 → 정렬 → 상위 N개

임베딩 캐시 흐름:
  embedBatchWithCache(texts)
    → SHA-256(text) → hash
    → embedding_cache에서 (provider, model, hash) 조회
    → 캐시 미스만 provider.embedBatch()
    → 새 결과 embedding_cache INSERT
    → 순서 보존 병합 반환
...
리인덱싱 중 서비스 중단 없이 원본 DB를 보존하는 Atomic Reindex 흐름:

1. temp DB 생성 (`dbPath + '.reindex.tmp'`)
2. 원본 memories 테이블에서 모든 메모리 읽기
3. 새 provider로 `embedBatchWithCache()` → temp DB에 chunks + vec0 삽입
4. temp DB close
5. `renameSync(tmpPath, originalPath)` — atomic swap
6. 실패 시 temp 삭제, 원본 보존

> atomic swap은 같은 파일시스템 내에서만 보장된다. WAL/SHM 파일도 함께 처리해야 한다.

### 5.10 임베딩 캐시 통합 흐름

`embedBatchWithCache` 함수 설계:

1. 각 텍스트에 대해 `SHA-256(text)` → hash 계산
2. `embedding_cache` 테이블에서 `(provider, model, hash)` 조합으로 조회
3. 캐시 히트인 항목은 캐시에서 임베딩 반환
4. 캐시 미스인 항목만 모아서 `provider.embedBatch()` 호출
5. 새로 생성된 임베딩을 `embedding_cache`에 INSERT
6. 원래 입력 순서에 맞춰 캐시 히트 + 새 결과를 병합하여 반환
...
| ------------------ | --------------------------------------------------------------------------- | --------------------------- |
| Phase 1 (types)    | `MemoryEntry`, `SearchResult`, `MarketData`, `Alert`, `AlertCondition` 타입 | 테이블 인터페이스 정의      |
...
| 패키지       | 버전            | 용도                         |
| ------------ | --------------- | ---------------------------- |
| `sqlite-vec` | `0.1.7-alpha.2` | 벡터 유사도 검색 SQLite 확장 |

> **참고**: `node:sqlite`는 Node.js 22+ 내장 모듈이므로 별도 설치 불필요.

### storage/package.json 의존성
...
| 16  | sqlite-vec ESM 로드      | `SELECT vec_version()` 실행                    | 버전 문자열 반환                 |
| 17  | vec0-chunks 삭제 동기화  | 메모리 삭제 후 memory_chunks_vec 확인          | 수동 CASCADE로 vec0 행도 삭제됨  |
| 18  | trigram 한국어 FTS       | "삼성전자" 저장 → "삼성" 검색                  | 부분 매칭 결과 반환              |
| 19  | 임베딩 캐시 HIT          | 동일 텍스트 2회차 임베딩 요청                  | API 미호출, 캐시에서 반환        |
| 20  | StorageAdapter 타입 정합 | `createStorage()` 반환값 할당                  | StorageAdapter 타입 호환 확인    |
| 21  | Alert 타입 정합          | alerts 테이블 CRUD에 @finclaw/types Alert 사용 | 타입 불일치 없음                 |
...
| 새 의존성          | `sqlite-vec@0.1.7-alpha.2`          |
| 예상 구현 시간     | 8-12시간                            |
