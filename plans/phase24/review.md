...
테스트: **1322 → 1361 (+39 신규)**. routing.test 33 + fallback floor 7 + skill-metadata 4 + 기타 −5 (중복 제거).
...
| `e8b4bcc` | 리뷰     | review.md 31 Todo 결과 + 발견된 이슈 정리                                             |
| `[다음]`  | 보정     | **도구 필터링 + fetch 키워드 — chat → Opus 승격 부작용 해결 (실 환경 검증으로 발견)** |
...
### ✅ 해결됨 — 도구 필터링 보정 + fetch 키워드 추가 (테스트 중 발견 → 즉시 처리)

**원래 증상 (1차 검증 시 발견):** Discord 실 채팅 4 메시지 모두 `claude-opus-4-7` 로 라우팅. 일반 chat 도 `tool_min` 승격으로 Opus 강제.

```
이전: msg-1 "안녕"     → role=chat, A=sonnet, C=opus → opus (tool_min)
이전: msg-2 "AAPL 얼마야?" → role=chat, A=sonnet, C=opus → opus (tool_min)
```
...
#### 적용한 해결책 (커밋 `a4ba6f5` 예정)

**1. 도구 필터링 — `resolveModelForRequest` 가 effectiveTier 보다 높은 minModel 도구를 LLM 노출에서 제외**

- effectiveTier = `hint ?? role.preferred` (도구 무관)
- `respectMinModel=true` (기본): tool.minModel > effectiveTier 면 필터 → LLM 이 못 봄 → 호출 자체 불가능 → 안전 보장
- `respectMinModel=false`: 필터 비활성, 이전 B6 escalation 동작 (안전 우선, 비용 ↑)
- adapter / agent.ts 가 `decision.allowedToolNames` 로 toolDefinitions 필터링 → runner 에 전달

**2. executeForTui 에 `inferRole(input.userMessage)` 적용** — chat.send 도 메시지 키워드로 role 추론. role='chat' 강제는 의도 무시 가능성.

**3. `inferRole` 에 fetch 키워드 추가** (커밋 `[다음]`):
...
| 메시지           | role      | 모델         | filteredCount                       |
| ---------------- | --------- | ------------ | ----------------------------------- |
| "AAPL 얼마야?"   | **fetch** | **Haiku** ⚡ | 2 (portfolio + analyze_market 제외) |
| "환율 알려줘"    | **fetch** | **Haiku** ⚡ | 2                                   |
| "달러-원" (단독) | chat      | Sonnet       | 1                                   |
| "AAPL 분석해줘"  | analysis  | Opus         | 0 (전체 노출)                       |

**비용 절감:** 4 메시지 기준 Opus 4× ($0.20) → Haiku 2× + Sonnet 1× + Opus 1× ($0.062) = **~70% 절감**

**라우팅 매트릭스 (최종):**

| 사용자 의도         | role     | 모델   | 도구 노출 (14 중) |
| ------------------- | -------- | ------ | ----------------- |
| 시세/가격/환율/차트 | fetch    | Haiku  | 11 (haiku-min)    |
| 일반 채팅           | chat     | Sonnet | 13 (~ sonnet-min) |
| 명시적 분석         | analysis | Opus   | 14 (전체)         |

#### 트레이드오프

- ❌ **잠재적 false negative**: "AAPL 어때?", "이 회사 봐줘" 같이 의도가 모호한 메시지 → chat → analyze_market 미노출 → Sonnet 의 자체 추론 + 일반 도구로 답변. 실측 결과 (msg-3 "AAPL 최근 이슈 정리해줘") 답변 품질 OK.
- ❌ **단어 매칭 한계**: "달러-원", "유로화" 같은 통화 단독 표현은 fetch 키워드 미매칭. Phase 25 에서 통화 키워드 정교화 검토 (단 "원래" 등 흔한 단어 충돌 주의).
- ✅ **명시적 회피 경로 존재**: 사용자가 분석 원하면 "분석해줘" 키워드 또는 `modelHint='opus'` 지정. 한국어 자연 표현으로 충분.
...
pnpm format                         # oxfmt clean, 595 files
pnpm test                           # 1361 passed (149 test files)
```
...
### 실 환경 검증 (Discord 봇 / Docker 컨테이너 — 보정 후)

| 시나리오                              | 결과 | 로그 증거                                                                                              |
| ------------------------------------- | ---- | ------------------------------------------------------------------------------------------------------ |
| 부팅 시 routing 테이블 출력           | ✅   | `event: 'routing.loaded', table: { fetch:'haiku', chat:'sonnet', analysis:'opus', summarize:'haiku' }` |
| 시세 조회 ("AAPL 얼마야?")            | ✅   | `role: 'fetch', chosenModel: 'claude-haiku-4-5-20251001', filteredCount: 2`                            |
| 환율 조회 ("환율 알려줘")             | ✅   | `role: 'fetch', chosenModel: 'claude-haiku-4-5-20251001'`                                              |
| 일반 채팅 ("안녕")                    | ✅   | `role: 'chat', chosenModel: 'claude-sonnet-4-6', filteredCount: 1`                                     |
| 정리 요청 ("AAPL 최근 이슈 정리해줘") | ✅   | `role: 'chat', Sonnet, 13 도구로 정리 답변 생성 (analyze_market 미사용)`                               |
| 분석 키워드 ("분석해줘")              | ✅   | `role: 'analysis', chosenModel: 'claude-opus-4-7', filteredCount: 0 (전체 노출, analyze_market 호출)`  |
| reason 문자열 디버깅 가능성           | ✅   | `A=haiku, C=haiku, hint=none → haiku (role)` 형태로 모든 결정 추적 가능                                |
| Discord 봇 로그인 + 도구 등록         | ✅   | FinClaw#5522, market/news/alert tools 모두 registered                                                  |
| Gateway 3000 포트                     | ✅   | healthy 상태로 Up                                                                                      |
| 비용 절감                             | ✅   | 4 메시지 실측: Opus 4× ($0.20) → Haiku 2 + Sonnet 1 + Opus 1 ($0.062) = **~70% 절감**                  |
...
- [x] 10 코드 커밋 + 4 문서 + 1 리뷰 커밋
- [x] 단위 테스트 +39 (1322 → 1361)
- [x] 실 환경 부팅 + Discord 라우팅 4 메시지 1차 검증 → 부작용 발견
- [x] **도구 필터링 + fetch 키워드 보정 — 1차 검증 부작용 즉시 해결**
- [x] 실 환경 2차 검증 — 4 메시지 (얼마/환율/달러-원/분석) 모두 의도대로 라우팅
...
- [ ] FETCH_KEYWORDS 정교화 (통화/티커 단독 표현 처리) — Phase 25
...
5. **실 환경 검증의 ROI** — Discord 4 메시지 보내고 로그 분석하는 30분 작업이 plan 의 핵심 결정 (B6) 의 실용적 부작용을 노출. 보정 후 ~70% 비용 절감 달성. **테스트 직전 직접 사용해보는 단계는 단위 테스트와 별개로 필수**.
6. **단계별 키워드 휴리스틱의 충분성** — 사용자 의도를 LLM 에 분류시키는 2-pass 방식 대신, 한국어 키워드 ~10개로 fetch/chat/analysis 분류. 추가 LLM 호출 0회. msg-4 "달러-원" 같은 false negative 는 있지만 비용 ↑ 보다 안전한 default (Sonnet 으로 대응 가능).
