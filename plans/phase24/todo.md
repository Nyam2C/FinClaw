...
- **Deprecated 모델 ID 일괄 교체** ✅ (커밋 `76ce3d8`)
...
- **도구 필터링 + fetch 키워드 추가** ✅
  - **발견:** Discord 4 메시지 1차 검증에서 모든 chat 이 Opus 로 라우팅됨 (`tool_min` 승리). `analyze_market` (minModel=opus) 한 도구만으로 floor=opus 강제.
  - **해결:**
    1. `resolveModelForRequest` 가 `effectiveTier = hint ?? role.preferred` 보다 높은 minModel 도구를 LLM 노출에서 필터 (`allowedTools` 반환).
    2. `executeForTui` 도 `inferRole(input.userMessage)` 적용 — chat.send 도 메시지 키워드로 role 추론.
    3. `inferRole` 에 fetch 키워드 추가 (`얼마/가격/시세/주가/환율/차트/price/quote/rate/chart`).
  - **검증 (2차 Discord):**
    - "AAPL 얼마야?" → fetch → **Haiku** (이전 Opus, 10× 절감)
    - "환율 알려줘" → fetch → **Haiku**
    - "AAPL 분석해줘" → analysis → Opus (analyze_market 도구 사용)
    - 4 메시지 비용: $0.20 → $0.062 (**~70% 절감**)
  - **트레이드오프:** "달러-원" 같은 통화 단독 표현은 fetch 미매칭 → chat → Sonnet. 사용자 명시 분석 키워드 또는 modelHint='opus' 로 회피 가능.
...
- **FETCH_KEYWORDS 정교화** — 현재 키워드 리스트는 한국어 명시 표현 위주. "달러-원", 단독 티커 ("BTC", "AAPL") 등은 미매칭. Phase 25 에서 통화/티커 패턴 매칭 정교화 검토 (단 "원래" 등 흔한 단어 충돌 주의).
